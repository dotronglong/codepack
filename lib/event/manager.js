...
function onAsyncCompleted(event, err, results) {
  const eventFailed  = `${event.name}:${EventManager.FIRE_FAILED}`
    , eventCompleted = `${event.name}:${EventManager.FIRE_COMPLETED}`
  if (err !== null) {
    if (err instanceof Event.StopPropagation) {
      event.name = eventCompleted
    } else {
      event.name  = eventFailed
      event.error = err
  ...
  fire(event) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (listener.limit !== Listener.LIMIT_NONE) {
        ...
        this.events[name].listeners[i].limit--
      }

      if (listener.limit === 0) {
        // remove this listener
        this.events[name].listeners.splice(i, 1)
      }

      parallels.push((callback) => {
        listener.runner(event)
        if (event.stopped) {
          callback(Event.StopPropagation, event)
        } else {
          callback(null, event)
        }
      })
